      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
        ...
        <div className="bg-gradient-to-br from-stone-50/95 via-stone-100/90 to-stone-200/85 backdrop-blur-md border border-stone-300/40 rounded-xl sm:rounded-2xl p-6 sm:p-8 mb-6 sm:mb-8 shadow-2xl ring-1 ring-stone-200/30 transition-all duration-500">
          <div className="text-center mb-4">
            ...
            <p className="text-stone-600 text-sm">Sell your harvested vegetables by quality tier</p>
          </div>
        ...
        <div className="bg-gradient-to-br from-stone-50/95 via-stone-100/90 to-stone-200/85 backdrop-blur-md border border-stone-300/40 rounded-xl sm:rounded-2xl p-6 sm:p-8 shadow-2xl ring-1 ring-stone-200/30 transition-all duration-500">
          <div className="text-center mb-8">
            ...
            <p className="text-stone-600 text-sm">Set prices and quantities for each product by quality tier</p>
          </div>
          
          <div className="space-y-6">
            {salesItems.map((item) => {
              return (
                <div key={item.productId} className="bg-white/60 backdrop-blur-sm rounded-xl p-6 border border-stone-200/30 shadow-lg">
                  {/* Product Header */}
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    />
                    <div>
                      <h3 className="text-xl font-bold text-stone-800">{item.productName}</h3>
                      <p className="text-sm text-stone-600">Available: {item.availableQuantity} kg</p>
                    </div>
                  </div>

                  {/* Quality Tier Selector */}
                  <div className="mb-6">
                    <label className="block text-sm font-medium text-stone-700 mb-3">Quality Tier</label>
                    <div className="relative">
                      <select
                        value={item.selectedTier}
                        onChange={(e) => handleTierChange(item.productId, e.target.value as 'starter' | 'basic' | 'premium' | 'max')}
                        className="w-full px-4 py-3 border border-stone-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-stone-400 focus:border-transparent bg-white appearance-none cursor-pointer"
                      >
                        {tiers.map((tier) => (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    <div>
                      <label className="block text-sm font-medium text-stone-700 mb-2">Price ($/kg)</label>
                      <input
                        type="number"
                        min="0"
                        step="0.01"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-stone-700 mb-2">Quantity to Sell</label>
                      <input
                        type="number"
                        min="0"
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="text-center sm:text-left">
                      <div className="text-sm text-stone-600">Estimated Profit</div>
                      <div className="text-2xl font-bold text-green-600">${item.estimatedProfit.toFixed(2)}</div>
                    </div>
                    <button
        ...
        <div className="bg-gradient-to-br from-stone-50/95 via-stone-100/90 to-stone-200/85 backdrop-blur-md border border-stone-300/40 rounded-xl sm:rounded-2xl p-6 sm:p-8 mt-6 sm:mt-8 shadow-2xl ring-1 ring-stone-200/30 transition-all duration-500">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-stone-800 mb-2">Warehouse Inventory by Quality Tier</h2>
            <p className="text-stone-600 text-sm">Current stock of harvested vegetables by quality</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {tiers.map((tier) => (
              <div key={tier.id} className="bg-white/40 backdrop-blur-sm rounded-xl p-6 border border-stone-200/30 shadow-lg">
                <div className="text-center mb-6">
                  <div className="text-xl font-bold text-stone-800 mb-2 capitalize">{tier.name}</div>
                  <div className={`w-12 h-1 bg-gradient-to-r ${tier.color} mx-auto rounded-full`}></div>
                </div>
                <div className="space-y-4">
                  {products.map((product) => (
                    <div key={product.id} className="flex items-center justify-between p-3 bg-white/60 rounded-lg border border-stone-200/40">
